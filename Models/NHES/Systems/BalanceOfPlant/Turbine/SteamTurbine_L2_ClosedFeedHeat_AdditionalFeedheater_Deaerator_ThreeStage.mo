...
model SteamTurbine_L2_ClosedFeedHeat_AdditionalFeedheater_Deaerator_ThreeStage
  "Two stage BOP model"
  ...
  Fluid.Vessels.IdealCondenser Condenser(
    ...
    annotation (Placement(transformation(extent={{342,2},{322,22}})));
  ...
  TRANSFORM.Fluid.Machines.SteamTurbine MPT(
    nUnits=1,
    energyDynamics=TRANSFORM.Types.Dynamics.DynamicFreeInitial,
    Q_units_start={3e7},
  ...
  Modelica.Blocks.Sources.RealExpression FWTank_level(y=deaerator.level)
    "level"
    annotation (Placement(transformation(extent={{258,-156},{270,-144}})));
  Modelica.Blocks.Sources.Constant const1(k=3)
    annotation (Placement(transformation(extent={{308,-128},{294,-114}})));
  TRANSFORM.Controls.LimPID Pump_Speed(
    ...
    xi_start=0,
    y_start=0.01)
    annotation (Placement(transformation(extent={{282,-128},{268,-114}})));
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort
                                       sensor_T3(redeclare package Medium =
        Modelica.Media.Water.StandardWater,
    p_start=100000,
    T_start=373.15)                                    annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=180,
        origin={116,-134})));
  TRANSFORM.Fluid.Machines.Pump                pump_SimpleMassFlow1(
    ...
    use_port=true)                                       annotation (
      Placement(transformation(
        ...
        rotation=180,
        origin={166,-134})));
  Fluid.HeatExchangers.Generic_HXs.NTU_HX_SinglePhase MainFeedwaterHeater2(
    NTU=15,
    K_tube=data.MainFeedHeater_K_tube,
    K_shell=data.MainFeedHeater_K_shell,
    redeclare package Tube_medium = Modelica.Media.Water.StandardWater,
    ...
    V_Shell=data.MainFeedHeater_V_shell,
    p_start_tube=100000,
    h_start_tube_inlet=1e5,
    h_start_tube_outlet=1e5,
    p_start_shell=1500000,
    h_start_shell_inlet=400e3,
    h_start_shell_outlet=300e3,
    dp_init_tube=0,
    ...
    m_start_tube=80,
    m_start_shell=5)
    annotation (Placement(transformation(extent={{192,-202},{212,-182}})));
  Fluid.HeatExchangers.Generic_HXs.NTU_HX_SinglePhase MainFeedwaterHeater3(
    NTU=1,
    K_tube=data.MainFeedHeater_K_tube,
    K_shell=data.MainFeedHeater_K_shell,
    redeclare package Tube_medium = Modelica.Media.Water.StandardWater,
    ...
    p_start_tube=200000,
    h_start_tube_inlet=1e5,
    h_start_tube_outlet=2e5,
    p_start_shell=200000,
    h_start_shell_inlet=1e5,
    h_start_shell_outlet=1e5,
    dp_init_tube=0,
    dp_init_shell=0,
    Q_init=2e6,
    m_start_tube=80,
    m_start_shell=3)
    annotation (Placement(transformation(extent={{224,-174},{244,-194}})));
  ...
  TRANSFORM.Fluid.Valves.ValveLinear LPT_Bypass1(
    ...
    m_flow_nominal=10)                            annotation (Placement(transformation(
        ...
        rotation=90,
        origin={112,-42})));
  Modelica.Blocks.Sources.Constant const5(k=102000)
    annotation (Placement(transformation(extent={{326,-64},{306,-44}})));
  StagebyStageTurbineSecondary.Control_and_Distribution.Timer             timer(
      Start_Time=1e-2)
    annotation (Placement(transformation(extent={{238,-46},{230,-38}})));
  Modelica.Blocks.Sources.Constant const8(k=0)
    annotation (Placement(transformation(extent={{238,-58},{230,-50}})));
  Modelica.Blocks.Math.Add         add2
    annotation (Placement(transformation(extent={{208,-52},{196,-40}})));
  TRANSFORM.Fluid.Machines.Pump_PressureBooster
                                           firstfeedpump1(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    use_input=false,
    ...
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        rotation=0,
        origin={266,-188})));
  TRANSFORM.Fluid.Volumes.MixingVolume FeedwaterMixVolume2(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    p_start=200000,
    use_T_start=false,
    h_start=1e6,
  ...
  TRANSFORM.Fluid.Machines.Pump_PressureBooster
    ...
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        rotation=0,
        origin={312,-188})));
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance R_feedwater2(R=0,
      redeclare package Medium = Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        rotation=180,
        origin={76,-200})));
  Modelica.Blocks.Sources.RealExpression
                                   realExpression(y=383)
    annotation (Placement(transformation(extent={{-324,-138},{-310,-126}})));
  StagebyStageTurbineSecondary.Control_and_Distribution.Timer             timer1(
      Start_Time=1e-2)
    annotation (Placement(transformation(extent={{-186,-128},{-178,-120}})));
  Modelica.Blocks.Sources.Constant const2(k=0.1)
    annotation (Placement(transformation(extent={{-186,-142},{-178,-134}})));
  ...
  Modelica.Blocks.Sources.Ramp ramp1(
    ...
    offset=5e-5,
    startTime=3e4)
    annotation (Placement(transformation(extent={{-284,-104},{-264,-84}})));
  PrimaryHeatSystem.HTGR.VarLimVarK_PID PID2(
    ...
    yMin=-0.099,
    xi_start=1500)
           annotation (Placement(transformation(extent={{-242,-140},{-222,-120}})));
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance R_feedwater3(R=0,
      redeclare package Medium = Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        rotation=180,
        origin={262,-168})));
  Modelica.Blocks.Sources.Ramp ramp2(
    ...
    offset=5e-7,
    startTime=1.5e4)
    annotation (Placement(transformation(extent={{256,-18},{276,2}})));
  ...
  Modelica.Blocks.Sources.RealExpression
                                   realExpression1(y=deaerator.medium.p)
    annotation (Placement(transformation(extent={{230,-94},{256,-74}})));
initial equation
...
equation
  ...
  connect(sensorBus.Steam_Temperature, sensor_T1.T) annotation (Line(
      points={{-30,100},{-2,100},{-2,52},{-22,52},{-22,42.16}},
      color={239,82,82},
  ...
  connect(TCV.port_b, sensor_T1.port_a) annotation (Line(
      ...
      color={0,127,255},
      thickness=0.5));
  connect(MPT.portHP, tee.port_1) annotation (Line(
      points={{76,40},{68,40}},
      color={0,127,255},
      thickness=0.5));
  ...
  connect(sensorBus.Feedwater_Temp, sensor_T2.T) annotation (Line(
      ...
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(HPT.shaft_b,MPT. shaft_a) annotation (Line(
      points={{14,34},{20,34},{20,26},{52,26},{52,24},{72,24},{72,20},{76,20},{
          76,34}},
      color={0,0,0},
      pattern=LinePattern.Dash));
  ...
  connect(actuatorBus.opening_TCV, TCV.opening) annotation (Line(
      points={{30.1,100.1},{-4,100.1},{-4,56},{-50,56},{-50,46.4}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(sensor_p.port, TCV.port_a)
    annotation (Line(points={{-64,50},{-64,46},{-58,46},{-58,40}},
                                                          color={0,127,255}));
  connect(LPT_Bypass.port_b, FeedwaterMixVolume.port_a[1])
    annotation (Line(points={{58,-12},{58,-18},{36,-18},{36,-66}},
                                                          color={0,127,255}));

  connect(generator1.portElec, sensorW.port_a) annotation (Line(points={{220,14},
          {218,14},{218,0}},                       color={255,0,0}));
  connect(sensorW.port_b, portElec_b) annotation (Line(points={{198,0},{172,0},
          {172,12},{146,12},{146,0},{160,0}},        color={255,0,0}));
  connect(sensorBus.Steam_Pressure, sensor_p.p) annotation (Line(
      points={{-30,100},{-80,100},{-80,60},{-70,60}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(header.port_b[1], TCV.port_a)
    annotation (Line(points={{-106,40},{-58,40}}, color={0,127,255}));
  connect(TBV.port_a, TCV.port_a) annotation (Line(points={{-120,74},{-92,74},{
          -92,40},{-58,40}},   color={0,127,255}));
  ...
  connect(actuatorBus.Divert_Valve_Position, LPT_Bypass.opening) annotation (
      Line(
      points={{30,100},{30,54},{72,54},{72,-2},{66,-2}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(Moisture_Separator.port_Liquid, FeedwaterMixVolume1.port_a[1])
    annotation (Line(points={{30,36},{30,-50},{52,-50},{52,-62},{64,-62},{64,
          -70}}, color={0,127,255}));
  connect(FeedwaterMixVolume1.port_b[1], R_entry1.port_a) annotation (Line(
        points={{64,-82},{64,-94},{88,-94},{88,-101}}, color={0,127,255}));
  ...
  connect(R_entry1.port_b, deaerator.steam) annotation (Line(points={{88,-115},
          {89,-115},{89,-147}}, color={0,127,255}));
  connect(FWTank_level.y,Pump_Speed. u_m)
    annotation (Line(points={{270.6,-150},{275,-150},{275,-129.4}},
                                                           color={0,0,127}));
  connect(const1.y,Pump_Speed. u_s) annotation (Line(points={{293.3,-121},{
          283.4,-121}},       color={0,0,127}));
  ...
  connect(Pump_Speed.y, pump_SimpleMassFlow1.inputSignal) annotation (Line(
        points={{267.3,-121},{166,-121},{166,-127}}, color={0,0,127}));
  connect(MainFeedwaterHeater3.Tube_out, MainFeedwaterHeater2.Tube_in)
    annotation (Line(points={{224,-188},{212,-188}}, color={0,127,255}));
  connect(LPT1.shaft_b, generator1.shaft_a)
    annotation (Line(points={{162,34},{220,34}}, color={0,0,0}));
  connect(MPT.shaft_b, LPT1.shaft_a)
    annotation (Line(points={{96,34},{142,34}}, color={0,0,0}));
  connect(MPT.portLP, tee1.port_2)
    annotation (Line(points={{96,40},{102,40}}, color={0,127,255}));
  connect(tee1.port_3, LPT_Bypass1.port_a) annotation (Line(points={{112,30},{
          112,-6},{98,-6},{98,-32},{112,-32}}, color={0,127,255}));
  connect(tee1.port_1, LPT1.portHP)
    annotation (Line(points={{122,40},{142,40}}, color={0,127,255}));
  connect(LPT1.portLP, Condenser.port_a) annotation (Line(points={{162,40},{338,
          40},{338,22},{339,22}}, color={0,127,255}));
  connect(add2.u1,timer. y) annotation (Line(points={{209.2,-42.4},{219.32,
          -42.4},{219.32,-42},{229.44,-42}},                    color={0,0,127}));
  connect(add2.u2,const8. y) annotation (Line(points={{209.2,-49.6},{220,-49.6},
          {220,-54},{229.6,-54}},                                        color=
          {0,0,127}));
  connect(sensor_T3.port_b, pump_SimpleMassFlow1.port_b)
    annotation (Line(points={{126,-134},{156,-134}}, color={0,127,255}));
  connect(MainFeedwaterHeater2.Tube_out, pump_SimpleMassFlow1.port_a)
    annotation (Line(points={{192,-188},{186,-188},{186,-134},{176,-134}},
        color={0,127,255}));
  connect(firstfeedpump1.port_b, MainFeedwaterHeater3.Tube_in)
    annotation (Line(points={{256,-188},{244,-188}}, color={0,127,255}));
  connect(MainFeedwaterHeater2.Shell_out, port_b1) annotation (Line(points={{
          212,-194},{216,-194},{216,-160},{130,-160}}, color={0,127,255}));
  connect(LPT_Bypass1.port_b, FeedwaterMixVolume2.port_a[1]) annotation (Line(
        points={{112,-52},{112,-78},{178,-78},{178,-94},{190,-94}}, color={0,
          127,255}));
  connect(FeedwaterMixVolume2.port_b[1], MainFeedwaterHeater3.Shell_in)
    annotation (Line(points={{190,-106},{224,-106},{224,-182}}, color={0,127,
          255}));
  connect(firstfeedpump2.port_b, firstfeedpump1.port_a)
    annotation (Line(points={{302,-188},{276,-188}}, color={0,127,255}));
  connect(firstfeedpump2.port_a, Condenser.port_b) annotation (Line(points={{
          322,-188},{334,-188},{334,-124},{332,-124},{332,2}}, color={0,127,255}));
  connect(R_feedwater2.port_b, MainFeedwaterHeater2.Shell_in) annotation (Line(
        points={{83,-200},{83,-196},{160,-196},{160,-194},{192,-194}}, color={0,
          127,255}));
  connect(R_feedwater2.port_a, MainFeedwaterHeater1.Shell_out) annotation (Line(
        points={{69,-200},{-10,-200},{-10,-48}}, color={0,127,255}));
  connect(add1.u1, timer1.y)
    annotation (Line(points={{-164,-124},{-177.44,-124}}, color={0,0,127}));
  connect(const2.y,add1. u2) annotation (Line(points={{-177.6,-138},{-177.6,
          -136},{-164,-136}},
                     color={0,0,127}));
  connect(realExpression.y, PID2.u_s) annotation (Line(points={{-309.3,-132},{
          -277,-132},{-277,-130},{-244,-130}}, color={0,0,127}));
  connect(PID2.u_m, sensor_T3.T) annotation (Line(points={{-232,-142},{-180,
          -142},{-180,-72},{-170,-72},{-170,-64},{0,-64},{0,-62},{12,-62},{12,
          -46},{96,-46},{96,-94},{116,-94},{116,-137.6}}, color={0,0,127}));
  connect(ramp1.y, PID2.prop_k) annotation (Line(points={{-263,-94},{-178,-94},
          {-178,-118.6},{-224.6,-118.6}}, color={0,0,127}));
  connect(PID2.y, timer1.u) annotation (Line(points={{-221,-130},{-204,-130},{
          -204,-124},{-186.8,-124}}, color={0,0,127}));
  connect(MainFeedwaterHeater3.Shell_out, R_feedwater3.port_a) annotation (Line(
        points={{244,-182},{244,-175},{255,-175},{255,-168}}, color={0,127,255}));
  connect(add2.y, LPT_Bypass1.opening) annotation (Line(points={{195.4,-46},{
          128,-46},{128,-42},{120,-42}}, color={0,0,127}));
  connect(const5.y, PID1.u_s)
    annotation (Line(points={{305,-54},{286,-54}}, color={0,0,127}));
  connect(PID1.y, timer.u) annotation (Line(points={{263,-54},{178,-54},{178,
          -42},{238.8,-42}}, color={0,0,127}));
  connect(realExpression1.y, PID1.u_m) annotation (Line(points={{257.3,-84},{
          176,-84},{176,-66},{274,-66}}, color={0,0,127}));
  connect(R_feedwater3.port_b, firstfeedpump1.port_a) annotation (Line(points={
          {269,-168},{269,-178},{276,-178},{276,-188}}, color={0,127,255}));
  connect(ramp2.y, PID1.prop_k) annotation (Line(points={{277,-8},{178,-8},{178,
          -42.6},{266.6,-42.6}}, color={0,0,127}));
...
end SteamTurbine_L2_ClosedFeedHeat_AdditionalFeedheater_Deaerator_ThreeStage;
