...
model Dual_Pipe_Model_Therminol
...
for i in 1:nX loop
      ...
      hc_vap[i,1] = 1;
      hc_turb[i,1] = TRANSFORM.Math.spliceTanh(max(hc_boil[i,1],hc_cond[i,1]),hc_liq[i,1],0-0.05,0.005);
  ...
  hc_turbint[i,1] = hc_cond[i,1];
   ...
   //  hc[i,1] = TRANSFORM.Math.spliceTanh(hc_turb[i,1],hc_lam[i,1],Re[i,1]-3650,135);
     ...
     hc_turb[i,2] = TRANSFORM.Math.spliceTanh(hc_turbint[i,2],hc_liq[i,2],0-0.05,0.05);
